...
namespace NetcodePlus.Demo
{
    ...
    public class Explorer : SNetworkPlayer
    {
        ...
        private void UpdateOwner()
        {
            ...
            sync_state.position = rigid.position;
        ...
        private void UpdateNotOwner()
        {
            ...
            Vector3 offset = sync_state.position - rigid.position; //Is the character position out of sync?

            if (offset.magnitude > sync_threshold)
                rigid.position = Vector3.MoveTowards(rigid.position, sync_state.position, sync_interpolate * Time.fixedDeltaTime);
            ...
            if(angle > sync_threshold)
                rigid.rotation = Quaternion.Slerp(rigid.rotation, Quaternion.LookRotation(sync_state.facing, Vector3.up), sync_interpolate * Time.fixedDeltaTime);
        ...
        {
            Vector3 face = (pos - transf.position);
            face.y = 0f;
        ...
        {
            float offset = 0.1f;
            return PhysicsTool.DetectGround(transf, rigid.position + Vector3.up * offset, ground_dist + offset);
        }
